...
| `email.username` | `REDACTED_SMTP_USERNAME` | 发件人邮箱 |
| `email.password` | `REDACTED_SMTP_PASSWORD` | SMTP 授权码（非 QQ 密码） |
...
**成绩页新增「电子成绩单」导出（发送至邮箱）**

- ✨ 成绩查询页工具栏「等级考试」「成绩分析」旁新增「电子成绩单」按钮，点击弹出导出表单
- ✨ 完整对齐教务端表单选项：显示方式（相同科目最好成绩 / 全部考试成绩）、导出类型（中文成绩单 / 成绩单英文）、课程类型（全部 / 主修 / 辅修 / 微专业）、是否打印本专业排名
- ✨ 接收邮箱自动带出账号邮箱，支持前端格式校验
- 🔧 新增接口 `POST /api/user/grades/transcript/export`，后端转发至教务 `/jsxsd/dzqz/printXscjkByQzPdf`
- 🔧 新增配置项 `jwc.{campus,webvpn}.transcript_url`（campus 与 webvpn 两种模式均已配置）
- ⚡ **异步提交设计**：教务端生成 PDF 并发送邮件耗时可达数十秒至数分钟，同步等待会撞上前端/网关超时。后端在完成参数校验与会话准备后立即返回「已受理」，真正的导出请求转由后台 goroutine 执行，用户无需保持页面等待
- ⚡ 教务绑定失效时会立即返回 `40013`（会话准备阶段同步完成），前端自动弹出重新绑定窗口
- 🐛 关键修正：实测发现教务端受理任务后**会返回 404 通用错误页**（"抱歉，您访问的页面不存在"），但成绩单仍能正常生成并送达邮箱。原判定逻辑会把这种情况误记为"请求失败"，现已识别为"已受理"
- 🐛 **关键修正：修复服务器上"显示已受理但收不到邮件"的静默失败**。根因是 Redis 中的陈旧教务会话（服务重启后残留，TTL 看似正常但教务端已单方面失效）被直接复用，导出请求被踢回登录页。新增会话预检 `probeSessionAlive`：当会话来自缓存时，先探测一次成绩接口确认可用，失效则强制重登
  - 判据为实测确定：教务端未登录时固定返回紧凑 JSON `{"flag1":2,"msgContent":"请先登录系统"}`，会话有效时同一接口返回成绩数据，绝不会出现该文案（**注意**：教务端对无效 cookie 也会重新下发 `bzb_jsxsd`，故"cookie 值是否变化"是伪判据，不可用）
  - 仅对缓存命中的会话探测（新登录的会话必然新鲜），避免给正常请求增加额外往返
  - 新增单元测试 `internal/modules/grade/session_probe_test.go` 固化判据
- 🔧 nginx 为 `/api/user/grades/transcript/` 单独放宽超时（300s）作为双保险

### 2026-09-08

**教务绑定失效自动识别与重新绑定弹窗**

- 🔧 新增错误码 `40013 CodeJwcBindExpired`（教务绑定已失效：密码变更 / 会话过期）
- 🔧 新增错误码 `40014 CodeJwcSessionExpired`：教务系统返回 HTTP 401（登录态失效）时不再被当作普通请求失败静默降级为旧缓存数据，而是明确判定为绑定失效
- ✨ 成绩、课表、考试、教评四条数据链路在「自动重登失败」「被踢回登录页」或「HTTP 401」时统一返回 40013，并失效过期会话（页面改版导致的解析失败不误判）
- ✨ 前端新增可关闭的「教务绑定已失效」弹窗：显示学号、输入教务密码即可更新绑定，成功后自动重试当前标签页数据
- ✨ 更新绑定时若教务要求短信验证，自动转入已有 MFA 验证码流程
- 🐛 修复：数据请求认证失败时不再清除数据库中的绑定（此前会把学号一并抹掉导致无法定向提示），只失效会话缓存
- 🐛 修复：管理后台公告/使用须知的「编辑」「删除」按钮无反应（按钮动作未接线），现已修复并全流程验证
