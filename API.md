...
`POST /api/user/bind/mfa/send`

绑定（`POST /api/user/bind`）返回 `40011`（教务系统要求多因素认证）时调用，触发短信验证码。
...
```json
{
  "code": 0,
  "message": "success",
  "data": { "challenge_id": "xxx", "masked_phone": "138****8888", "message": "验证码已发送" }
}
```

### 2.9 绑定教务系统-提交短信验证码

`POST /api/user/bind/mfa/verify`
...
### 2.10 教务会话-发送短信验证码（多因素认证）

`POST /api/user/mfa/send`

**任意查询页面**命中 `40011` 时调用。`spwd` 可省略 —— 省略时后端自动使用数据库里已保存的教务密码发码，
因此前端可以「零输入」触发验证，用户只需输入收到的短信码。

**请求体（可省略）：**
```json
{
  "spwd": "教务系统密码（可省略，省略则用已保存的密码）"
}
```

**响应：** 同 2.8。

### 2.11 教务会话-提交短信验证码（多因素认证）

`POST /api/user/mfa/verify`

与 2.9 的区别：不修改绑定关系，也不清除刚拿到的会话缓存；校验通过后重试原查询即可。
...
`POST /api/user/grades/transcript/export`

由教务系统生成官方电子成绩单 PDF 并发送到指定邮箱。
...
| 字段 | 可选值 | 默认 | 说明 |
|------|--------|------|------|
| `selShowType` | `1` / `3` | `1` | `1`=显示相同科目最好的考试成绩，`3`=显示相同科目全部的考试成绩 |
| `tpa` | `1` / `2` | `1` | `1`=中文成绩单，`2`=成绩单(英文) |
| `kclx` | `""` / `0` / `1` / `9` | `""` | 课程类型：空=全部，`0`=主修，`1`=辅修，`9`=微专业 |
| `dybzypm` | `1` / `0` | `1` | 是否打印本专业排名 |
| `mail` | — | — | 接收成绩单的邮箱（必填，需符合邮箱格式） |
...
**说明：** 教务端在请求内同步生成 PDF 并发送邮件，耗时可达数十秒甚至数分钟。
后端采用**异步提交**：完成校验与会话准备后立即返回，真正的导出请求在后台执行。
因此 `code: 0` 表示"已受理"，并非"邮件已送达"；用户在几分钟后查收邮箱即可。
若教务绑定已失效，接口会返回 `40013`，前端应引导用户重新输入教务密码。

**会话预检（2026-09-18 起为所有教务数据查询通用）：**
Redis 中的会话缓存 TTL 为 1 小时，但教务端会话寿命可能更短；
服务重启后 Redis 也可能保留教务端已单方面失效的旧会话。
若直接使用，请求会被踢回登录页或返回"未登录"响应，而接口把它当成"没有数据"——
用户看到的是空白课表、"暂无数据"，且坏 cookie 会被反复复用直到 Redis TTL 到期。

因此成绩、课表、考试、等级考试成绩等**所有教务数据查询**，在会话来自缓存时都会先探测一次
成绩接口确认会话可用；判据是教务端未登录时的标准响应
`{"flag1":2,"msgContent":"请先登录系统"}`（实测确认，统一实现在 `internal/service/session_alive.go`）。
探测到失效即清除会话并用库中保存的密码重新登录（必要时走短信验证）；
重登后仍不可用则返回 `40014`，各业务模块会统一转换为 `40013` 返回，
前端据此弹出「重新输入教务密码」引导，而不是显示"没有数据"。

> 本接口（电子成绩单）的差异：缓存会话失效时强制重登，重登后仍不可用返回 `50100`。
