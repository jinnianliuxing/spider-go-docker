...
**会话预检：** Redis 中的会话缓存 TTL 为 1 小时，但教务端会话寿命可能更短；
服务重启后 Redis 也可能保留教务端已单方面失效的旧会话。
若直接提交，请求会被踢回登录页，而前端仍收到"已受理"——用户永远收不到邮件的静默失败。
因此当会话来自缓存时，接口会先探测一次成绩接口确认会话可用；
判据是教务端未登录时的标准响应 `{"flag1":2,"msgContent":"请先登录系统"}`（实测确认）。
探测失败即强制重登；重登后仍不可用则返回 `50100` 并提示稍后重试。
